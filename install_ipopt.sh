...
./$srcdir/ThirdParty/Blas/get.Blas
...
./$srcdir/ThirdParty/Lapack/get.Lapack
mkdir -p build && cd build
../configure --prefix=$prefix --disable-shared --with-pic \
...
./$srcdir/ThirdParty/ASL/get.ASL

# MUMPS
cd $srcdir/ThirdParty/Mumps
./$srcdir/ThirdParty/Mumps/get.Mumps

# build everything
cd $srcdir
./$srcdir/configure --prefix=$prefix coin_skip_warn_cxxflags=yes \
    --with-blas="$prefix/lib/libcoinblas.a -lgfortran" \
